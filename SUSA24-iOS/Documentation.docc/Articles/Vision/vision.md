...
Vision Framework 의 RecognizeDocumentsRequest를 주로 활용하여 문서 이미지에서 테이블, 리스트, 텍스트를 분석합니다.
``VisionModel`` 에서 관리되는 State에 문서 이미지를 분석하고 주소를 추출하는 기능이 포함되어 있습니다.


Vision Framework를 활용하여 문서 스캔 기능을 제공합니다.
...
- 실시간 감지: 촬영 전 문서 경계를 시각적으로 확인하여 촬영 품질 향상시킵니다.
- 렌즈 얼룩 감지: 렌즈 오염 시 토스트로 안내하여 선명한 이미지 확보합니다.
- 주소 자동 추출: 수사관이 다량의 문서에서 주소 정보를 수동으로 입력하는 번거로움을 해소했습니다.
- 좌표 검증: 문서 이미지에서 주소를 자동 인식하여 정확한 좌표로 변환합니다.
- 중복 감지: 중복 주소 감지를 통해 방문 빈도 파악 시 핀 추가 지원합니다.
...
1. 사용자가 **[카메라 스캔 버튼을 탭]**하면 [``ScanLoadView``]로 [이동]합니다.
2. [``ScanLoadView``]가 [``CameraSession``] 을 이용해 실시간으로 [분석]합니다.
3. 분석 중 로딩 애니메이션을 표시합니다.
4. 분석 완료 후 [``ScanListView``] 로 이동하여 추출된 주소 목록을 표시합니다.
5. 사용자가 주소 목록 중 추가할 리스트를 선택하고 카테고리를 지정하여 핀을 추가합니다.
...
실시간 문서 감지
- 현재 실제 구현 기능은 렌즈 얼룩 여부 로직 적용된 상태입니다.
- Vision 프로세서 초기화 및 에러 처리에 대한 방안을 필요로 합니다.

문서 이미지 분석
- iOS 26+ 이상에서만 사용 가능합니다.
- 손글씨 및 저해상도 이미지에 대한 처리방안이 없습니다.
- 테이블 구조가 불규칙 할 경우에 대한 고도화 필요합니다. (현재는 Fallback)
- API 호출 시 네트워크 처리 필요합니다.
