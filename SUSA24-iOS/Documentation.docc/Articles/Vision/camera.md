...
State는 문서 이미지를 분석하고 주소를 추출하는 기능이 포함되어 있습니다.

``AVFoundation`` 활용하여 카메라 기능을 제공합니다.
1. 카메라 프리뷰 제공 및 촬영
...
1. 사용자가 [카메라 버튼을 탭]하면 [``CameraView``]로 [이동]합니다.
2. [``CameraView``] 에서 카메라 권한을 확인하고 프리뷰가 [시작]됩니다.
3. 촬영 버튼을 탭하면 사진이 촬영되고 썸네일이 [업데이트]됩니다.
4. [썸네일을 탭]하면 [``PhotoDetailsView``]에서 촬영된 사진을 [조회]하거나 [삭제]합니다.
5. [스캔 버튼을 탭]하면 [``ScanLoadView``] 로 [이동]합니다.
...
- 현재 촬영된 문서 이미지는 저장되지 않음, 메모리 관리 목적으로 최대 10장까지 촬영 가능합니다.
- Vision 분석은 현재 매 10프레임마다 처리합니다.(3fps)
- 백그라운드 진입 시 카메라 세션이 자동으로 일시정지 처리됩니다.
- 줌 범위는 현재 강제로 고정되어있습니다.
...
- 촬영 중에도 문서 인식 및 텍스트 인식 검증 로직 검토 필요합니다.
- 이미지 전처리 기능(회전, 밝기 보정 등)에 대한 기능 추가 필요합니다.
- 자동 문서 경계 감지 후 자동 촬영 기능 검토 필요합니다.

### 기술 부채

- PhotoDetailsFeature 에서 CameraModel 을 참조하는 의존성 문제 해결이 필요합니다.
- 문서 감지 오버레이 코드는 현재 사용하고 있지 않습니다.
