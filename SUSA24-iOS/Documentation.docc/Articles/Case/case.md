...
4. 사건 추가/편집 완료 시 저장 로직이 실행되고, 화면이 pop 됩니다. (``CaseAddFeature/Action/addCaseButtonTapped`` + coordinator.pop())
...
1) 사건 리스트 로딩
- ``CaseListView``가 .task { store.send(.onAppear) }로 시작합니다.
- ``CaseListFeature/Action/onAppear`` → ``CaseRepository/fetchCases()`` 호출 → ``CaseListFeature/Action/loadCases(_:)``로 State 업데이트합니다.
- View는 ``CaseListFeature/State/cases``를 기반으로 카드 리스트를 렌더링합니다.

2) 사건 생성 (신규)
- ``CaseAddView``에서 사용자가 폼을 채우고 버튼 탭:
...
- ``CaseAddFeature/Action/addCaseButtonTapped`` 발생 시
    - editingCaseId == nil → ``CaseRepository/createCase(model:imageData:phoneNumber:)`` 실행

3) 사건 편집
- ``CaseAddFeature/Action/onAppear``에서 ``CaseAddFeature/State/editingCaseId``가 있으면
    ``CaseRepository/fetchCaseForEdit(for:)`` 호출 후 -> ``CaseAddFeature/Action/setExistingCase(_:phoneNumber:profileImagePath:)``로 폼을 채웁니다.
- 이미지 처리
    - 편집 모드에서 기존 이미지는 ``CaseAddFeature/State/existingProfileImagePath``로 유지합니다.
    - 사용자가 새 이미지를 고르면 FullScreenPhotoPicker → ``CaseAddFeature/Action/setProfileImage(_:)``로 Data만 업데이트합니다.
- 저장 시
    - ``CaseRepository/updateCase(model:imageData:phoneNumber:)``에서
    - imageData == nil이면 기존 이미지 유지 / imageData != nil이면 기존 파일 삭제 후 새 파일 저장, CoreData에는 “경로”만 저장합니다.

4) 사건 삭제
- ``CaseListFeature/Action/deleteTapped(item:)``에서 ``CaseRepository/deleteCase(id:)`` 실행
- 삭제 완료 후 ``CaseRepository/fetchCases()``를 재호출해서 리스트를 갱신합니다.
- deleteCase 내부에서 Suspect의 기존 이미지 파일도 함께 삭제합니다. (``ImageFileStorage/deleteProfileImage(at:)``)

5) 쇼케이스용 목데이터(기지국/핀) 추가
- ``CaseCard``의 Menu에서 “기지국 추가 / 핀 추가” 선택 시 ``CaseListFeature``에서 CoreData context를 직접 가져와 ``LocationRepository``를 생성합니다.
...
| :--- | :--- |
| ``CaseListFeature/State/selectedTab`` | 탭 선택 상태 (현재는 .allCase만 유효) |
| ``CaseListFeature/State/cases`` | 렌더링할 사건 목록 | 
| ``CaseListFeature/State/targetCaseIdForCellLog`` | 기지국 목데이터 대상 케이스 |
| ``CaseListFeature/State/isShowingOverwriteAlert`` / ``CaseListFeature/State/isShowingSuccessAlert`` | 기지국 overwrite/완료 Alert 제어 |
| ``CaseListFeature/State/targetCaseIdForPinData`` | 핀 목데이터 대상 케이스 |
| ``CaseListFeature/State/isShowingPinDataOverwriteAlert`` / ``CaseListFeature/State/isShowingPinDataSuccessAlert`` | 핀 overwrite/완료 Alert 제어 |

2. CaseAddFeature.State

| Variable Name | Description | 
| :--- | :--- |
| ``CaseAddFeature/State/editingCaseId`` | nil이면 신규, 값이 있으면 편집 모드 | 
| ``CaseAddFeature/State/caseName``, ``CaseAddFeature/State/caseNumber``, ``CaseAddFeature/State/suspectName``, ``CaseAddFeature/State/crime``, ``CaseAddFeature/State/suspectPhoneNumber`` | 폼 입력값 |
| ``CaseAddFeature/State/suspectProfileImage`` | 사용자가 새로 선택한 이미지 Data | 
| ``CaseAddFeature/State/existingProfileImagePath`` | 편집 모드에서 기존 이미지 경로 |
| ``CaseAddFeature/State/isFormComplete`` | 5개 필드가 모두 채워졌는지 |
...
- **목데이터 주입 책임 위치**: ``CaseListFeature``에서 직접 CoreData context를 가져와 ``LocationRepository``를 생성합니다. (추후 DI/Repository 추상화로 정리 여지)
