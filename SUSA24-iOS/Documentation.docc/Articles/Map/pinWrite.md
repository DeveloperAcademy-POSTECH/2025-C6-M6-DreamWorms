...
지도에서 특정 위치를 탭하여 수사 관련 장소 정보를 핀으로 등록하고 관리합니다.
핀에는 이름, 색상, 카테고리(거주지/범행지/기타)를 지정할 수 있으며, 형사 노트를 추가하여 수사 메모를 기록할 수 있습니다.
...
- **장소 기록**: 수사 중 중요 장소(거주지, 범행지, 기타)를 지도에 표시하여 시각화합니다.
- **카테고리 분류**: 장소 유형별 분류로 수사 정보 체계적 관리합니다.
- **색상 구분**: 7가지 색상으로 핀 간 시각적 구분합니다.
- **메모 기록**: 형사 노트로 장소별 수사 내용 기록합니다.
...
![PinWrite Sequence](pin-sequence.svg)
...
![PinWrite Sequence](pin-write-state.svg)

### NoteWriteFeature 상태

![PinWrite Sequence](pin-note-state.svg)

---

## 6. 의존성 다이어그램 (Dependency Diagram)

![PinWrite Sequence](pin-dependency.svg)
---
